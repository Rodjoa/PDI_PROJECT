...
def analise_samples(base_folder, obj_folder, frames, vis = False):
    ...
    for idx, frame_filename in enumerate(sorted_masks_list):
        ...
        start = time()
        length, height = GeoMetrics_instance.get_fish_length_and_height(frame = fish_frame,
                                          depth_map= depth_map,
                                          pcd = pcd,
    ...
    if PLOT_LENGTHS:
        ...
        # Graficar las longitudes con el MSE
        ...
        # Agregar el MSE al título del gráfico
        plt.title('Video: {} | Length estimations\nMSE: {:.2f}'.format(SVO_FOLDER, mse))
        plt.show()
